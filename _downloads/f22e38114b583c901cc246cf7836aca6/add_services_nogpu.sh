...
if [ ! -f "$CHECKPOINT_FILE" ] || [ "$(cat $CHECKPOINT_FILE)" == "" ]; then
	...
	/opt/jupyterhub/bin/python3 -m pip install scikit-learn tensorflow tensorflow_datasets matplotlib pandas gradio seaborn geopandas rasterio sentinelhub xarray ipyleaflet folium mapclassify openeo ddc_utility streamlit leafmap boto3 contextily cdsapi cartopy netcdf4 h5netcdf 
...
EOF
	...
	export SUPERSET_ADMIN_PASSWORD='<your_superset_password>'
